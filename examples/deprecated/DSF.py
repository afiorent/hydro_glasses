...
Q_list = np.array(
    list(
        set(
            tuple(sorted(l))
            for l in [[i, j, k] for i in range(12) for j in range(12) for k in range(12)]
        )
    )
)
...
for iq, Q in enumerate(Q_list):
    ...
    x, y = lanczos.spectrum(
        A=dynmat_tf, v=phi_tf, k=100, omega_array=omega_array, eta=eta, is_tf=True
    )
    spectrum[iq] = y
